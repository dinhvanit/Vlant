...
import { Edit, MessageCircle, UserPlus, Check, X, Clock, UserMinus } from 'lucide-react';
...
const ProfilePage = () => {
  ...
  const handleSendRequest = () => {
    if (!profile) return;
    dispatch(sendFriendRequest({ userId: profile._id, username: profile.username }));
  };
  ...
  const handleUnfriend = () => {
    if (!profile) return;
    if (window.confirm(`Are you sure you want to unfriend ${profile.username}? This action cannot be undone.`)) {
  ...
  const ActionButtons = () => {
    ...
    switch (profile.friendStatus) {
      ...
      case 'friends':
        return (
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Button variant="destructive" onClick={handleUnfriend} className="rounded-full px-6">
      ...
      case 'request_sent':
        return (
          <Button variant="outline" disabled className="rounded-full px-6 cursor-not-allowed">
      ...
      case 'request_received':
        return (
          <div className="flex gap-3">
            <Button onClick={handleAcceptRequest} className="rounded-full px-6 bg-green-600 hover:bg-green-700">
              <Check className="w-4 h-4 mr-2"/> Accept
            </Button>
            <Button onClick={handleDeclineRequest} variant="destructive" className="rounded-full px-6">
      ...
      <Card className="bg-card/95 backdrop-blur-md border-border/50 rounded-3xl mb-6">
        <CardContent className="p-8">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-muted-foreground mb-6 max-w-md">{profile.bio || "This wanderer has not shared their story yet."}</p>
            <div className="h-10"> {/* Container để nút không bị nhảy layout */}
              <ActionButtons />
            </div>
          </div>
        </CardContent>
            ...
            <CardContent>
              {profile.friends?.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {/* Sẽ cần API riêng để fetch chi tiết bạn bè */}
                  <p className="text-muted-foreground col-span-full">Friend list display coming soon...</p>
                </div>
              ) : (
                <p className="text-muted-foreground text-center p-8">No friends to display.</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
        <TabsContent value="lanterns" className="mt-6">
           <Card className="bg-card/95 backdrop-blur-md border-border/50 rounded-3xl">
             <CardContent className="p-8 text-center text-muted-foreground">
               ...
               <p className="mt-4">This user's lanterns are anonymous and float freely in the night sky.</p>
             </CardContent>
           </Card>
        </TabsContent>
      </Tabs>
